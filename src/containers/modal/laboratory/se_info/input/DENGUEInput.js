            ...
            <CRow className="ml-1 mr-1 p-0">
                <CCol md="6" className="p-1">
                    <CRow className="m-0 p-0">
                        <FormControl fullWidth className={clsx(classes.margin, "m-0 p-0")} variant="outlined" margin='dense'>
                            <CLabel className={clsx(classes.labelText, "mb-0 ml-2")}>DENGUE NS1</CLabel>
                            <Select
                                native
                                value={JSON.stringify(props.seroData.dengue.result) || ''}
                                onChange={
                                    props.index !== undefined && props.index !== null
                                        ? props.handleSelectChange('dengue', 'result', props.index)
                                        : props.handleSelectChange('dengue', 'result')
                                }
                            >
                ...
                <CCol md="6" className="p-1">
                    <FormControl fullWidth className={clsx(classes.margin, "m-0 p-0")} variant="outlined" margin='dense'>
                        ...
                        <Select
                            native
                            defaultValue={JSON.stringify(props.seroData.dengue.referenceLabId) || ""}
                            onChange={
                                props.index !== undefined && props.index !== null
                                    ? props.handleSelectChange('dengue', 'referenceLabId', props.index)
                                    : props.handleSelectChange('dengue', 'referenceLabId')
                            }
                        >
                            <option value={JSON.stringify('')}>--</option>
                            {
