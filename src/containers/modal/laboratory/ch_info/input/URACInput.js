...
const URACInput = (props) => {
    ...
    let uaconv = '0.00'
    if (props.chemData.urac.conventional !== '') uaconv = props.chemData.urac.conventional
            ...
            <CRow className="ml-1 mr-1 p-0">
                <CCol md="3" className="p-1">
                    <FormControl fullWidth className={clsx(classes.margin, "m-0 p-0")} variant="outlined">
                        ...
                        <CInput
                            type="number"
                            value={props.chemData.urac.result}
                            onChange={
                                props.index !== undefined && props.index !== null
                                    ? props.handleChange('urac', 'result', props.index)
                                    : props.handleChange('urac', 'result')
                            }
                        />
                    </FormControl>
                </CCol>
                ...
                <CCol md="2" className="p-1 mt-4">
                    <strong className={clsx(classes.labelText)}>{'155 - 428'}</strong>
                </CCol>
                <CCol md="3" className="p-1">
                    <CLabel className={clsx(classes.labelText, "mb-0 ml-2")}>Conventional</CLabel><br />
                    <CLabel className={"mb-0 ml-2 font-weight-bold"}>{uaconv}</CLabel>
                    <CLabel className={"mb-0 ml-2"}>mg/dl</CLabel>
                </CCol>
                <CCol md="2" className="p-1 mt-3">
                    <strong className={clsx(classes.labelText)}>{'2.60 - 7.18'}</strong>
                </CCol>
