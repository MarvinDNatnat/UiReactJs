...
export class ChemistryTable extends Component {
    componentDidMount() {
        this.props.onRef(this);
        var _this = this;
        $(document).ready(function () {
            $('#chemistryTable').DataTable({
                columns: [
                    ...
                    },
                    {
                        ...
                        render: (cellData, type, row, meta) => {
                            return `<button
                                    ...
                                    `;
                            // commented, will move to Quest Quality
                            // <button
                            //     type="button"
            ...
            $('#chemistryTable tbody').on('click', 'button.results', function () {
                var row = $(this).parents('tr');
                var rowData = $('#chemistryTable').DataTable().row(row).data();
                _this.props.openChemistryModal(rowData, row);
            });

            $('#chemistryTable tbody').on('click', 'button.view', function () {
                var row = $(this).parents('tr');
                var rowData = $('#chemistryTable').DataTable().row(row).data();
                _this.props.viewChemistryModal(rowData, row);
            });
            // commented, will move to Quest Quality
            // $('#chemistryTable tbody').on('click', 'button.show', function () {
            //     var row = $(this).parents('tr');
            //     var rowData = $('#chemistryTable').DataTable().row(row).data();
            //     _this.props.onShowEmailModal(rowData);
            // });

            $('#chemistryTable tbody').on('click', 'button.print', function () {
                var row = $(this).parents('tr');
                var rowData = $('#chemistryTable').DataTable().row(row).data();
                _this.props.onPrintChemistry(rowData.transaction.transactionid, rowData.id, rowData.status, false);
            });
        });
    }
    ...
    componentWillUnmount() {
        ...
        $('#chemistryTable').DataTable().destroy(true);
        this.props.onClearLaboratoryList('CH');
    }

    reloadTableData = (data) => {
        const table = $('#chemistryTable').DataTable();
        table.clear().draw();
        if (data.length > 0) {
            table.rows.add(data);
            table.draw();
    ...
    shouldComponentUpdate(nextProps) {
        if (nextProps !== undefined && nextProps.chemistryList !== undefined &&
            nextProps.chemistryList.length >= 0 &&
            nextProps.chemistryList.length !== this.props.chemistryList.length) {
            this.reloadTableData(nextProps.chemistryList);
        }
        return false;
    }

    addChemistryToTable = (chemistryData) => {
        const table = $('#chemistryTable').DataTable();
        table.row.add(chemistryData);
        table.draw();
    }

    updateChemistryToTable = (chemistryData, idx) => {
        const table = $('#chemistryTable').DataTable();
        table.row(idx).data(chemistryData).draw();
    }

    render() {
        return (
            <table id="chemistryTable" className="table table-bordered table-striped display">
            </table>
        )
    }
}
...
const mapStateToProps = (state) => {
    return {
        chemistryList: state.lab.chemistryList,
    }
};
...
export default connect(mapStateToProps, mapDispatchToProps)(ChemistryTable)
