...
export class HematologyTable extends Component {
    componentDidMount() {
        this.props.onRef(this);
        var _this = this;
        $(document).ready(function () {
            $('#hematologyTable').DataTable({
                columns: [
            ...
            $('#hematologyTable tbody').on('click', 'button.results', function () {
                var row = $(this).parents('tr');
                var rowData = $('#hematologyTable').DataTable().row(row).data();
                _this.props.openHematologyModal(rowData, row);
            });

            $('#hematologyTable tbody').on('click', 'button.view', function () {
                var row = $(this).parents('tr');
                var rowData = $('#hematologyTable').DataTable().row(row).data();
                _this.props.viewHematologyModal(rowData, row);
            });

            // $('#hematologyTable tbody').on('click', 'button.show', function () {
            //     var row = $(this).parents('tr');
            //     var rowData = $('#hematologyTable').DataTable().row(row).data();
            //     _this.props.onShowEmailModal(rowData, row);
            // });

            $('#hematologyTable tbody').on('click', 'button.print', function () {
                var row = $(this).parents('tr');
                var rowData = $('#hematologyTable').DataTable().row(row).data();
                _this.props.onPrintHematology(rowData.transaction.transactionid, rowData.id, rowData.status, false);
            });
        });
    }
    ...
    componentWillUnmount() {
        ...
        $('#hematologyTable').DataTable().destroy(true);
        this.props.onClearLaboratoryList('HE');
    }

    reloadTableData = (data) => {
        const table = $('#hematologyTable').DataTable();
        table.clear().draw();
        if (data.length > 0) {
            table.rows.add(data);
            table.draw();
    ...
    shouldComponentUpdate(nextProps) {
        if (nextProps !== undefined && nextProps.hematologyList !== undefined &&
            nextProps.hematologyList.length >= 0 &&
            nextProps.hematologyList.length !== this.props.hematologyList.length) {
            this.reloadTableData(nextProps.hematologyList);
        }
        return false;
    }

    addHematologyToTable = (hematologyData) => {
        const table = $('#hematologyTable').DataTable();
        table.row.add(hematologyData);
        table.draw();
    }

    updateHematologyToTable = (hematologyData, idx) => {
        const table = $('#hematologyTable').DataTable();
        table.row(idx).data(hematologyData).draw();
    }

    render() {
        return (
            <table id="hematologyTable" className="table table-bordered table-striped display">
            </table>
        )
    }
}
...
const mapStateToProps = (state) => {
    return {
        hematologyList: state.lab.hematologyList,
    }
};
...
export default connect(mapStateToProps, mapDispatchToProps)(HematologyTable)
